...
export default defineComponent({
  ...
  computed: {
     ...
     */
days():  number |string {
      return ('0' +Math.floor(this.totalMilliseconds / MILLISECONDS_DAY)).slice(-2);
    },
     ...
     */
    hours():  number |string {
      return ('0' +Math.floor((this.totalMilliseconds % MILLISECONDS_DAY) / MILLISECONDS_HOUR)).slice(-2);
    },
  ...
  watch: {
    $props: {
       ...
       */
      handler() {
        this.totalMilliseconds = this.now() - this.time;
  ...
  methods: {
    ...
    update() {
      if (this.counting) {
        this.totalMilliseconds = Math.max(0, this.now() - this.time);
      }
    },
